...
// GET /api/schedule — weekly schedule for a wing
router.get(
  '/',
  requireRole('supervisor', 'admin', 'nurse'),
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (!wingId) {
        res.status(400).json({
          error: 'wingId is required (query param or from user profile)',
      ...
      if (weekStartParam) {
        ...
        if (isNaN(weekStart.getTime())) {
          res.status(400).json({ error: 'Invalid weekStart date format', code: 'VALIDATION_ERROR' });
          return;
        }
      } else {
...
// GET /api/schedule/forecast — 7-day demand forecast
router.get(
  '/forecast',
  ...
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (days < 1 || days > 30) {
        res.status(400).json({ error: 'days must be between 1 and 30', code: 'VALIDATION_ERROR' });
        return;
      }
...
// POST /api/schedule/suggestions — AI staffing suggestions
router.post(
  '/suggestions',
  requireRole('supervisor', 'admin'),
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      if (isNaN(startDate.getTime()) || isNaN(endDate.getTime())) {
        res.status(400).json({ error: 'Invalid date format', code: 'VALIDATION_ERROR' });
        return;
      }
      ...
      res.json({
        ...
        analyzed: {
          ...
          daysAnalyzed: suggestions.length,
        },
      });
    } catch (error) {
...
// PUT /api/schedule/:staffId/:date — set/update shift
router.put(
  '/:staffId/:date',
  requireRole('supervisor', 'admin'),
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const schedule = await setShift(staffId, date, parsed.data.shiftType as ShiftType);
      res.json({ schedule });
    } catch (error) {
