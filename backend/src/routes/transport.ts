...
// POST /api/transport/jobs — create job
router.post(
  '/jobs',
  requireRole('nurse', 'supervisor', 'admin', 'transporter'),
...
// PATCH /api/transport/jobs/:id/accept — transporter accepts job
router.patch(
  '/jobs/:id/accept',
...
// PATCH /api/transport/jobs/:id/status — update job status
router.patch(
  '/jobs/:id/status',
  requireRole('transporter', 'supervisor', 'admin'),
  async (req: AuthRequest, res: Response): Promise<void> => {
    try {
      ...
      const job = await updateJobStatus(
        req.params.id,
        parsed.data.status as JobStatus,
        req.user!.userId
      );
...
// GET /api/transport/jobs/:id — job detail
...
// GET /api/transport/nemt — STUB: NEMT providers
router.get(
  '/nemt',
  requireRole('nurse', 'supervisor', 'admin'),
    ...
    res.json({
      providers: [
        ...
        },
        {
          ...
          available: true,
          avgEta: '35 minutes',
          services: ['standard', 'wheelchair', 'stretcher', 'bariatric'],
        },
        {
          id: 'veyo',
          name: 'Veyo',
...
// POST /api/transport/nemt/schedule — STUB: schedule external ride
router.post(
  '/nemt/schedule',
  requireRole('nurse', 'supervisor', 'admin'),
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    console.log('[NEMT STUB] Would schedule ride:', {
      patientId,
      destination,
      pickupTime,
    ...
    res.status(202).json({
      ...
      stub: true,
      mockResult: {
        rideId: `stub-${Date.now()}`,
        provider: provider || 'Lyft Health',
        eta: '30 minutes',
        status: 'confirmed',
        pickupTime,
        destination,
