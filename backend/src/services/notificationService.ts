...
export async function sendRoomReadyNotification(roomId: string, nurseId: string): Promise<void> {
  try {
    ...
    if (!room) {
      console.error(`[Notification] Room ${roomId} not found`);
      return;
    }
    ...
    if (!nurse) {
      console.error(`[Notification] Nurse ${nurseId} not found`);
      return;
    }
    ...
    await sendPushNotification(
      nurseId,
      'Room Ready',
      `Room ${room.roomNumber} in ${room.wing.name} is now clean and ready for patient assignment.`,
      {
        roomId,
...
export async function sendJobDispatchNotification(
  jobId: string,
  ...
  try {
    ...
    if (!job) {
      console.error(`[Notification] Job ${jobId} not found`);
      return;
    }

    const transporter = await prisma.staff.findUnique({
      where: { id: transporterId },
    });

    if (!transporter) {
      console.error(`[Notification] Transporter ${transporterId} not found`);
      return;
    }
...
export async function sendAlertNotification(
  ...
  await sendPushNotification(supervisorId, `Alert: ${alertType}`, message, {
    alertType,
    type: 'alert',
  });
}
