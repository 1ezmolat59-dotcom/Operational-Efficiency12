...
router.post('/login', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.json({
      token,
      refreshToken,
      user: { id: staff.id, name: staff.name, role: staff.role, wingId: staff.wingId, email: staff.email, wing: staff.wing },
    })
  } catch (error) {
...
router.post('/refresh', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const { data: staff } = await db.from('Staff').select('*').eq('id', decoded.userId).single()
    ...
    const payload: JwtPayload = {
      userId: staff.id, role: staff.role, wingId: staff.wingId, email: staff.email, name: staff.name,
    }
...
router.get('/me', authenticate, async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const { data: staff } = await db
      .from('Staff')
      .select('id, name, email, role, wingId, currentLocation, availabilityStatus, shiftStart, shiftEnd, createdAt, wing:Wing(*)')
